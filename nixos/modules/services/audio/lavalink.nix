...

{
    ...
    plugins = mkOption {
      type =
        types.either
          ...
          (types.listOf pluginModule)
          (types.attrsOf pluginModule);
      ...
      apply =
        ...
        if (lib.typeOf config) != "list" then
          ...
          let
            updatePlugin = plugin: {
              name = plugin.configName or plugin.dependency;
  ...
  config = mkIf cfg.enable {
    ...
    systemd.tmpfiles.settings."10-lavalink"."${cfg.home}/plugins".d = mkIf (cfg.plugins != { }) {
    ...
    systemd.services.lavalink = {
      ...
      environment.JAVA_TOOL_OPTIONS = cfg.jvmArgs;
      ...
      preStart =
        let
          ...
          pluginSymlinks = pipe cfg.plugins [
            attrValues
            (map symlinkPlugin)
            concatStrings
          ];

          config = lib.attrsets.recursiveUpdate cfg.settings {
          ...
          configFile = format.generate "application.yml" config;
        in
        ''
          ${pluginSymlinks}
      ...
      script = ''
        ${lib.optionalString (cfg.passwordFile != null) ''
          LAVALINK_SERVER_PASSWORD="$(cat "$CREDENTIALS_DIRECTORY/lavalink_password")"
        ''}
